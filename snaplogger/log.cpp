...
std::string         g_log_config_filename;
std::string         g_log_output_filename;
...
public:
    ...
    logger &        operator () (log_security_t const v)        { snap::NOTUSED(v); return *this; }
    logger &        operator () (char const * s)                { snap::NOTUSED(s); return *this; }
    logger &        operator () (wchar_t const * s)             { snap::NOTUSED(s); return *this; }
    logger &        operator () (char16_t const * s)            { snap::NOTUSED(s); return *this; }
    logger &        operator () (char32_t const * s)            { snap::NOTUSED(s); return *this; }
    logger &        operator () (std::string const & s)         { snap::NOTUSED(s); return *this; }
    logger &        operator () (std::wstring const & s)        { snap::NOTUSED(s); return *this; }
    logger &        operator () (std::u16string const & s)      { snap::NOTUSED(s); return *this; }
    logger &        operator () (std::u32string const & s)      { snap::NOTUSED(s); return *this; }
    logger &        operator () (advgetopt::option_info_ref const & s)
                                                                { snap::NOTUSED(s); return *this; }
    logger &        operator () (char const v)                  { snap::NOTUSED(v); return *this; }
    logger &        operator () (signed char const v)           { snap::NOTUSED(v); return *this; }
    logger &        operator () (unsigned char const v)         { snap::NOTUSED(v); return *this; }
    logger &        operator () (wchar_t const v)               { snap::NOTUSED(v); return *this; }
    logger &        operator () (char16_t const v)              { snap::NOTUSED(v); return *this; }
    logger &        operator () (char32_t const v)              { snap::NOTUSED(v); return *this; }
    logger &        operator () (signed short const v)          { snap::NOTUSED(v); return *this; }
    logger &        operator () (unsigned short const v)        { snap::NOTUSED(v); return *this; }
    logger &        operator () (signed int const v)            { snap::NOTUSED(v); return *this; }
    logger &        operator () (unsigned int const v)          { snap::NOTUSED(v); return *this; }
    logger &        operator () (signed long const v)           { snap::NOTUSED(v); return *this; }
    logger &        operator () (unsigned long const v)         { snap::NOTUSED(v); return *this; }
    logger &        operator () (signed long long const v)      { snap::NOTUSED(v); return *this; }
    logger &        operator () (unsigned long long const v)    { snap::NOTUSED(v); return *this; }
    logger &        operator () (float const v)                 { snap::NOTUSED(v); return *this; }
    logger &        operator () (double const v)                { snap::NOTUSED(v); return *this; }
    logger &        operator () (long double const v)           { snap::NOTUSED(v); return *this; }
    logger &        operator () (bool const v)                  { snap::NOTUSED(v); return *this; }
    logger &        operator () (void const * p)                { snap::NOTUSED(p); return *this; }
};
 ...
 *
 * \note
 * It is possible to set the \p progname parameter to an empty string
 * however the get_progname() function throws an exception when the
 * string is empty.
 *
 ...
 *
 * \exception logger_logic_exception
 * This exception is raised if the program name is not set (i.e. when it
 * is empty.)
 *
...
std::string get_progname()
{
    if(g_progname.empty())
    {
        throw logger_logic_exception( "g_progname undefined, please make sure to call set_progname() before calling any logger functions (even if with a fixed name at first)" );
    }
...
void configure_logfile(std::string const & logfile)
{
    ...
    if(logfile.empty())
    {
        throw logger_logic_exception( "No output logfile specified!" );
    }

    log4cplus::SharedAppenderPtr appender(new log4cplus::RollingFileAppender( logfile.c_str() ));
...
void configure_conffile(std::string const & filename)
{
    ...
    {
        throw logger_logic_exception("Cannot open logger configuration file [" + filename + "].");
    }
    ...
    //
    log4cplus::PropertyConfigurator::doConfigure(LOG4CPLUS_C_STR_TO_TSTRING(filename.c_str()));
...
logger::~logger()
{
    ...
    //
    // TODO: change control characters to \xXX in f_message
    //
    if( (g_logging_type == logging_type_t::UNCONFIGURED_LOGGER)
    ||  !logger_exists(log_security_t::LOG_SECURITY_SECURE == f_security ? "security" : "snap"))
    {
        ...
        if(sll != -1)
        {
            syslog(sll, "%s (%s:%s: %d)", f_message.c_str(), f_file, f_func, static_cast<std::int32_t>(f_line));
        }
    }
    else
    {
        ...
        if(log_security_t::LOG_SECURITY_SECURE == f_security)
        {
            ...
            //
            g_secure_logger.log(ll, LOG4CPLUS_C_STR_TO_TSTRING(f_message.c_str()), f_file, f_line, f_func);
        }
        else
        {
            g_logger.log(ll, LOG4CPLUS_C_STR_TO_TSTRING(f_message.c_str()), f_file, f_line, f_func);
...
logger & logger::operator () (wchar_t const * s)
{
    if(s != nullptr)
    {
        f_message += libutf8::to_u8string(s);
    }
    return *this;
}
...
logger & logger::operator () (std::u16string const & s)
{
    f_message += libutf8::to_u8string(s);
    return *this;
}
...
logger & logger::operator () (std::u32string const & s)
{
    f_message += libutf8::to_u8string(s);
    return *this;
}
...
logger & logger::operator () (advgetopt::option_info_ref const & s)
{
    f_message += std::string(s);
    return *this;
}
...
{
    f_message += std::to_string(static_cast<int>(v));
    return *this;
}
...
logger & logger::operator () (wchar_t const v)
{
    f_message += libutf8::to_u8string(v);
    return *this;
}


logger & logger::operator () (char16_t const v)
{
    f_message += libutf8::to_u8string(v);
    return *this;
}
...
logger & logger::operator () (char32_t const v)
{
    f_message += libutf8::to_u8string(v);
    return *this;
}
...
logger & logger::operator () (long double const v)
{
    f_message += std::to_string(v);
    return *this;
}
